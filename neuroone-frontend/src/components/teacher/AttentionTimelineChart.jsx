...
export function AttentionTimelineChart({ timelineData = [], thresholds = { low: 40, high: 70 }, height = 300 }) {
  ...
  const chartData = {
    labels,
    datasets: [
      // Área entre mínimo e máximo (preenchimento para mostrar range)
      {
        label: 'Variação',
        data: maxAttentionData,
        borderColor: 'transparent',
        backgroundColor: isDark ? 'rgba(59, 130, 246, 0.15)' : 'rgba(59, 130, 246, 0.1)',
        fill: '+1', // Preenche até o próximo dataset (mínimo)
        tension: 0.3,
        borderWidth: 0,
        pointRadius: 0,
        pointHoverRadius: 0,
      ...
      // Atenção média (linha principal - mais grossa)
      {
        label: 'Atenção Média',
